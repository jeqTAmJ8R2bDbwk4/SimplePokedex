...
fun Collection<Int>.consecutive(): List<ConsecutiveRange> {
...
fun <T: ViewHolder> Adapter<T>.notifyPositionChanged(positions: Collection<Int>, payload: Any?) {
    positions.sorted().consecutive().forEach { range ->
...
inline fun <reified T : Any> T.validateNonEmpty() {
    for (property in T::class.memberProperties) {
        property.findAnnotation<NonEmpty>() ?: continue
...
inline val ViewHolder.context: Context get() = itemView.context
...
fun Context.setThemePreferenceValue(value: String) {
    ...
    getSharedSettingsPreferences().edit { putString(preferenceThemeKey, value) }
}
...
fun Context.setDataSavingPreferenceValue(value: Boolean) {
    val preferenceDataSavingKey = getString(R.string.preference_data_saving_key)
    getSharedSettingsPreferences().edit { putBoolean(preferenceDataSavingKey, value) }
}
    ...
    return when(preferenceThemeEntryValue) {
        ...
        preferenceThemeEntryValueDark -> {
            ...
            throw IllegalArgumentException(
                "Unexpected preferenceThemeEntryValue: $preferenceThemeEntryValue"
            )
        }
    }
}
...
fun Snackbar.applyErrorColors() {
    val colorErrorContainer = MaterialColors.getColorOrNull(view.context, R.attr.colorErrorContainer)!!
    val colorOnErrorContainer = MaterialColors.getColorOrNull(view.context, R.attr.colorOnErrorContainer)!!
    setBackgroundTint(colorErrorContainer)
    setTextColor(colorOnErrorContainer)
}